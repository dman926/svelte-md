...
{:else}
	...
	{:else if token.type == 'link'}
		...
		<a href={token.href} data-md-token={token.start} data-md-type="link">{@render content()}</a>
	{:else if token.type == 'image'}
		<img src={token.href} alt={token.alt} data-md-token={token.start} data-md-type="image" />
